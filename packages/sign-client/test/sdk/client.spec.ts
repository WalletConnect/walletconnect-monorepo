...
describe("Sign Client Integration", () => {
  ...
  describe("connect", () => {
    ...
    it("connect (with old pairing)", async () => {
      const clients = await initTwoClients();
      await testConnectMethod(clients);
      console.log("booom connected");
      const { A, B } = clients;
      expect(A.pairing.keys).to.eql(B.pairing.keys);
      const { topic: pairingTopic } = A.pairing.get(A.pairing.keys[0]);
      console.log("connect the same clients again?", pairingTopic);
      await timeout(10000);
      await testConnectMethod(clients, {
        pairingTopic,
      });
      console.log("connected, deleting clients", pairingTopic);
      deleteClients(clients);
      console.log("deleting", pairingTopic);
    }, 60_000);
  });

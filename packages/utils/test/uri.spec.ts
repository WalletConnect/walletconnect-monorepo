...
import { TEST_SYMMETRIC_KEY, TEST_PAIRING_TOPIC, TEST_RELAY_OPTIONS } from "./shared";

const TEST_URI_PARAMS: UriParameters = {
  protocol: "wc",
  version: 2,
  topic: TEST_PAIRING_TOPIC,
  symKey: TEST_SYMMETRIC_KEY,
  relay: TEST_RELAY_OPTIONS,
};

const TEST_URI_STRING = `${TEST_URI_PARAMS.protocol}:${TEST_URI_PARAMS.topic}@${TEST_URI_PARAMS.version}?relay#protocol=${TEST_RELAY_OPTIONS.protocol}&symKey=${TEST_SYMMETRIC_KEY}`;
